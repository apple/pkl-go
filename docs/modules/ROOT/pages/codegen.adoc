...
Code generation is done through the `@pkl.go/gen.pkl` tool.
For usage instructions, consult the xref:ROOT:quickstart.adoc#codegen[codegen] section of the quickstart.

Go code may be generated from Pkl:

[source,bash]
----
pkl run @pkl.go/gen.pkl pkl/AppConfig.pkl
----
...
If a base path is not explicitly set, `@pkl.go/gen.pkl` will also look for a `go.mod` file in the current working directory and use the Go module name as the base path.
