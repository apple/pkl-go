...
Code generation is done through the `@pkl.golang/gen.pkl` tool.
...
If a base path is not explicitly set, `@pkl.golang/gen.pkl` will also look for a `go.mod` file in the current working directory and use the Go module name as the base path.
