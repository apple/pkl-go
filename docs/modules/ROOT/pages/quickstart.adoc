...
----
pkl run package://pkg.pkl-lang.org/pkl-go/pkl.golang@{version}#/gen.pkl pkl/AppConfig.pkl --base-path github.com/myorg/myteam
----
...
If using xref:main:language-reference:index.adoc#projects[a `PklProject`] to manage the `pkl.go` dependency, the tool's URI may be replaced with dependency notation:

[source,bash]
----
pkl run @pkl.golang/gen.pkl pkl/AppConfig.pkl --base-path github.com/myorg/myteam
----
...
[source,bash]
----
pkl run @pkl.golang/gen.pkl --help
----
