...
* Refactoring for performance and clarity of the parsing and comparison of versions.
* Fix unhandled error in the evaluator manager.
* Reduced lock contention in `atomicRandom`.
* Resolution of the path to `PklProject` files is is made platform independent by using `path.Join`.
