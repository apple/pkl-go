...
* Add support for `ExternalReader.WorkingDir` (https://github.com/apple/pkl-go/pull/256[#256]).

=== Fixes

* Add missing Go fields for `ref#Access.isProperty` and `ref#Access.isSubscript` (https://github.com/apple/pkl-go/pull/257[#257]).

=== Contributors ❤️

* https://github.com/bioball[@bioball]
* https://github.com/stoned[@stoned]
...
* Add support for `pkl.ref#Reference` (pr:https://github.com/apple/pkl-go/pull/238[]).
* Add support for configuring HTTP headers (pr:https://github.com/apple/pkl-go/pull/248[]).
* Add support for resolved evaluator settings (pr:https://github.com/apple/pkl-go/pull/249[]).

=== Fixes

* Correct `StderrLogger` to log to standard error instead of standard output (pr:https://github.com/apple/pkl-go/pull/226[]).
* Fix omission of codegen for elements of union types erased to `any` (pr:https://github.com/apple/pkl-go/pull/235[]).
* Omit hidden properties in parent modules during subclass code generation (pr:https://github.com/apple/pkl-go/pull/235[]).
* Decoding typed values into struct fields with type `any` as `pkl.Object` when no mapping is present (pr:https://github.com/apple/pkl-go/pull/232[]).
* Allow custom and external readers to produce "not found" results for nullable reads (`read?()`) (pr:https://github.com/apple/pkl-go/pull/230[]).
* Fix handling of the no proxy flag when applying evaluator options from a project (pr:https://github.com/apple/pkl-go/pull/228[]).
* Fix resource leak in `NewProjectEvaluator` (pr:https://github.com/apple/pkl-go/pull/249[])

=== Breaking Changes

* The `WithProjectEvaluatorSettings` method now applies the _resolved_ evaluator settings (introduced in Pkl 0.32). Users that need lower versions of Pkl should use `WithProjectEvaluatorSettingsLegacy`.

=== Miscellaneous

* Improve test coverage (pr:https://github.com/apple/pkl-go/pull/223[]).
* Reduce memory footprint of reflection operations (pr:https://github.com/apple/pkl-go/pull/237[]).
...
* https://github.com/NawafSwe[@NawafSwe]
...
* Fix loading `go.mod` to automatically determine `--base-path` (pr:https://github.com/apple/pkl-go/pull/218[]).
...
* Fix check for presence of generator-settings.pkl in current working directory (pr:https://github.com/apple/pkl-go/pull/214[]).
...
[IMPORTANT]
====
This release removes the `pkl-gen-go` binary and replaces it with a new xref:main:pkl-cli:index.adoc#cli-tools[`pkl:Command`]-based tool `@pkl.golang/gen.pkl` (pr:https://github.com/apple/pkl-go/pull/209[]).
...
* Upgrade to Go 1.26 (pr:https://github.com/apple/pkl-go/pull/208[]).
+
NOTE: pkl-go now requires Go 1.24.
* Refactor decoding logic to use switch statements (pr:https://github.com/apple/pkl-go/pull/186[]).
* Remove unnecessary build constraint (pr:https://github.com/apple/pkl-go/pull/190[]).
* Optimize map initialization in for improved performance (pr:https://github.com/apple/pkl-go/pull/192[]).
* Improve error reporting in unreachable decoding paths (pr:https://github.com/apple/pkl-go/pull/205[]).

=== Fixes

* Close channel in after evaluator manager interruption to prevent resource leaks (pr:https://github.com/apple/pkl-go/pull/191[]).
* Ensure registration order is stable in generated init.pkl.go files (pr:https://github.com/apple/pkl-go/pull/196[]).
...
* Fix project loading behavior in `pkl-gen-go` (https://github.com/apple/pkl-go/pull/181[#181]).
* Fix CLI flag overriding behavior in `pkl-gen-go` (https://github.com/apple/pkl-go/pull/182[#182])
...
* Enforce Pkl code formatting (https://github.com/apple/pkl-go/pull/178[#178]).
* Enforce Go code formatting (https://github.com/apple/pkl-go/pull/179[#179]).
